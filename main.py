...
def recommend_books(book_title, cosine_sim=cosine_sim):
    # Try to find the book index with case-insensitive, partial match
    matches = data[data['title'].str.lower().str.contains(book_title.lower(), na=False)]
    if matches.empty:
        print(f"No book found with title containing '{book_title}'. Please check the title and try again.")
        return []
    idx = matches.index[0]
